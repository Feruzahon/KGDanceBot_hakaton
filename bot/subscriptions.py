...
# to-do получить total_lessons автоматически
    ...
    def create_sub(self, call):
        ...
        self.sub_data[call.message.chat.id] = {'user_id':user_id}
    ...
    def get_total_lessons(self, message):
        ...
        data = {
                'user':self.sub_data[chat_id]['user_id'],
        ...
        try:
            ...
            if sub['group_days'] == 'mon/wed/fri':
                list_group_handler.groups_list_mon(chat_id, message.message_id)
            elif sub['group_days'] == 'tue/thu/sat':
                list_group_handler.groups_list_tue(chat_id, message.message_id)
            elif sub['group_days'] == 'sat/sun':
                list_group_handler.groups_list_sun(chat_id, message.message_id)
    ...
    def confirm_delete_sub(self, call):
        ...
        markup.row(
            ...
            types.InlineKeyboardButton('❌ Отмена', callback_data=f'group_user_{user_id}_{group_id}')
            )
    ...
    def delete_sub(self,call):
        sub_id = call.data.split('_')[2]
        chat_id=call.message.chat.id
        response=self.auth.delete(chat_id, f"subscription/delete_sub/{sub_id}/") 
    ...
    def show_my_subscriptions(self,call):
        ...
        try:
            response = self.auth.get(chat_id, f'subscription/get_my_sub/')
        ...
        f"🗓 Даты занятий:\n")
                    ...
                    text += "────────────────────\n\n"
                        # markup.add(types.InlineKeyboardButton(f'📅 {day.replace('-', '.')[:5]} {mark}', callback_data=f'/'))
    ...
    def show_childs_subscriptions(self, call):
        chat_id = call.message.chat.id
        try:
            response = self.auth.get(chat_id, f'subscription/get_child_sub/')
            if response.status_code == 200:
                subscriptions = response.json()
